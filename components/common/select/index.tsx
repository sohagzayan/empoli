...
const Select: React.FC<SelectProps> = ({
  ...
  return (
    <SelectPrimitive name={name} defaultValue={defaultValue}>
      <SelectTrigger
        className={`no-ring-shadow flex items-center gap-2 ${triggerClassName}`}
      >
        {icon && <span>{icon}</span>} {/* Render the common icon */}
        <SelectValue className="no-ring-shadow" placeholder={placeholder} />
      </SelectTrigger>
      <SelectContent
        className={`border-0 outline-none ring-0 focus:border-0 focus:outline-none focus:ring-0 ${contentClassName}`}
      >
        <SelectGroup>
          ...
          {options.map((option) => (
            <SelectItem
              key={option.value}
              value={option.value}
              className="border-0 outline-none ring-0 focus:border-0 focus:outline-none focus:ring-0"
